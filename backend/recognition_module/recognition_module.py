...
from recognition_module.recognize_function import recognize_text_from_images, recognize_text_from_image
...
class RecognitionModule:
    ...
    INPUT_CONFIG_FILE = os.path.join(BASE_DIR, "recognition_module", "configs", "default_config.yaml")
    ...
    @staticmethod
    def __extract_text_from_image(image_or_path, yolo_weights_path, ocr_models_directory, recog_network="best_accuracy", yolo_device="cpu", ocr_gpu=False):
        """
    ...
    @staticmethod
    def __extract_text_from_line(image_or_path, box, ocr_models_directory, recog_network="best_accuracy", yolo_device="cpu", ocr_gpu=False):
        # Проверяем, является ли входное изображение путем или numpy.ndarray
        if isinstance(image_or_path, str):
            image = cv2.imread(image_or_path)
            if image is None:
    ...
    @staticmethod
    def train(data, dataset_name, model_name_train, model_name_trained, num_iter, val_interval, batch_size, new_prediction, train_perc=0.9):
        ...
        try:
            ...
            output_file = RecognitionModule.generate_config(dataset_name, 'train', val_dir, model_name_train, model_name_trained,
    ...
    @staticmethod
    def generate_config(dataset_name, train_folder, val_dir, model_name_train, model_name_trained,
        ...
        update_yaml_parameters(RecognitionModule.INPUT_CONFIG_FILE, 
                               updates, 
                               output_file)
